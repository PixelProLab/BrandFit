...
export function BrandFitDropzone({ onFilesAccepted }: BrandFitDropzoneProps) {
  ...
  return (
    <section
      ...
      className={`relative overflow-hidden rounded-md border border-dashed bg-brand-surface p-6 shadow-brand-purple transition before:pointer-events-none before:absolute before:inset-0 before:bg-[linear-gradient(135deg,rgba(138,76,152,0.16),transparent_32%,rgba(244,168,77,0.08)_64%,rgba(222,85,143,0.12))] before:opacity-80 after:pointer-events-none after:absolute after:inset-0 after:bg-[linear-gradient(90deg,rgba(255,255,255,0.04)_1px,transparent_1px),linear-gradient(0deg,rgba(255,255,255,0.03)_1px,transparent_1px)] after:bg-[length:48px_48px] ${
        isDragging
    ...
    >
      ...
      <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="max-w-2xl">
          <h1 className="sr-only">BrandFit</h1>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <Image
              src="/BrandFit Logo.png"
              alt="BrandFit by Pixel Pro Lab logo"
              width={230}
              height={186}
              priority
          ...
          <p className="mt-5 text-base leading-7 text-brand-text-muted">
            The open-source engine for perfect image grids. Drop any combination of logos,
            icons, or graphics here to automatically trim, balance, and normalize them
            in milliseconds.
          </p>
        </div>
        <button
          ...
          className="h-12 rounded-md bg-brand-purple px-5 text-sm font-bold text-brand-text-main shadow-brand-purple transition hover:bg-brand-pink focus:outline-none focus:ring-2 focus:ring-brand-pink focus:ring-offset-2 focus:ring-offset-brand-bg"
        >
          Select logos
        </button>
      </div>
      <div className="relative z-10 mt-6 grid gap-3 text-sm text-brand-text-muted md:grid-cols-3">
      ...
      {message ? <p className="relative z-10 mt-4 text-sm text-brand-orange">{message}</p> : null}
    </section>
  );
}
